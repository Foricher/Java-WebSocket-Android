...
public abstract class WebSocketClient extends WebSocketAdapter implements Runnable, WebSocket {
	...
	private InetSocketAddress proxyAddress = null; //CF
		...
		try {
			...
			if( !socket.isBound() ) {
				...
				if (proxyAddress != null) {
					socket.connect( new InetSocketAddress( proxyAddress.getHostName(), proxyAddress.getPort() ), connectTimeout );
					waitConnectProxy = true ;
				} else {
					socket.connect( new InetSocketAddress( uri.getHost(), getPort() ), connectTimeout );
				}
				//CF--
			}
			socket.setTcpNoDelay(true) ; //CF
			istream = socket.getInputStream();
			ostream = socket.getOutputStream();
			if (waitConnectProxy) {
				String connectHttp = connectToHTTPProxy() ;
				ostream.write(connectHttp.getBytes(Charset.forName("UTF-8")));
			}
	...
	//CF++
	public String connectToHTTPProxy() {
	...
	public void setHttpProxy( InetSocketAddress proxyaddress ) {
